...
void CPlayFab::Setting_NewUserInfo()
{
	...
	for (_int i = 0; i < (_int)E_CHAR_NAME::Name_End; ++i)
	{
		CDataManager::GetInstance()->Get_UserInfo_Pointer()->bCharacter[i] = FALSE;
		CCharacterMgr::GetInstance()->Get_vecCharInfo()[i].bGetChar = FALSE;
	}

	CDataManager::GetInstance()->Get_UserInfo_Pointer()->bCharacter[(_int)E_CHAR_NAME::Name_Neru] = TRUE;
	CDataManager::GetInstance()->Get_UserInfo_Pointer()->bCharacter[(_int)E_CHAR_NAME::Name_Aru] = TRUE;
	CCharacterMgr::GetInstance()->Get_vecCharInfo()[(_int)E_CHAR_NAME::Name_Neru].bGetChar = TRUE;
	CCharacterMgr::GetInstance()->Get_vecCharInfo()[(_int)E_CHAR_NAME::Name_Aru].bGetChar = TRUE;
...
void CPlayFab::Save_Ranking_Leaderboard()
{
	if (FALSE == PlayFabClientAPI::IsClientLoggedIn())
	{
		return;
