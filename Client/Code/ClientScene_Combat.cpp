...
HRESULT CClientScene_Combat::Ready_Scene()
{
	...
	else if (E_WORLD_MOD::MOD_TOTAL == m_pWorld->Get_WorldMod())
	{
		CPlayFab::GetInstance()->Save_Ranking_Leaderboard();
	}
...
{  
	...
	else if (E_WORLD_MOD::MOD_MISSION == m_pWorld->Get_WorldMod())
	{
		...
		else if (L"임무 1 - 1.world" == m_pWorld->Get_WorldName())
		{
			CSound_Manager::GetInstance()->PlayBGM(L"BGM_일반전투_기계_메카니컬정글.mp3", 0.1f);
		}
		else if (L"임무 1 - 2.world" == m_pWorld->Get_WorldName())
		{
			CSound_Manager::GetInstance()->PlayBGM(L"BGM_일반전투_기계_메카니컬정글.mp3", 0.1f);
		}
		else if (L"임무 1 - 3.world" == m_pWorld->Get_WorldName())
		{
			CSound_Manager::GetInstance()->PlayBGM(L"BGM_일반전투_기계_메카니컬정글.mp3", 0.1f);
		}
...
_int CClientScene_Combat::Update_Scene(const _float & fTimeDelta)
{
	...
	{
		for (auto& pChar : *CGameManager::GetInstance()->Get_CurrentSquad_Strikers())
		{
			if (pChar)
			{
				pChar->Reset();
	...
	if (m_pWorld->Get_WorldName() == L"Binah.world")
	{
		...
		if (pBinah->Get_Boss_HP()->Get_Hp() < 300000)
		{
...
HRESULT CClientScene_Combat::Ready_LightInfo()
{
	...
	if (m_pWorld && m_pWorld->Get_WorldName() == L"쿠로.world")
	{
		T_GodLight tGodLight = T_GodLight();
		tGodLight.vPosition = _vec3(0.f, 20.f, -4.f);
		D3DXCOLOR vColor = CLightMgr::GetInstance()->Get_Light(0)->Diffuse;
		tGodLight.vColor = _vec4(vColor.r, vColor.g, vColor.b, vColor.a);
		tGodLight.fSize = 5.f;
		tGodLight.bSun = TRUE;

		_int iIndex = CLightMgr::GetInstance()->Create_GodLight_New(tGodLight.vPosition, tGodLight.vColor, tGodLight.fSize);
		CLightMgr::GetInstance()->Set_GodLight_Info(iIndex, tGodLight);
		CLightMgr::GetInstance()->Set_GodLight_Index_Main(iIndex);
	}
...
void CClientScene_Combat::Update_Fit_Camera(const _float & fTimeDelta)
{
	...
	// 그림자 거리 보정
	CLightMgr::GetInstance()->Set_CameraPos_Dis(fCharBack - fMonFront);
...
void CClientScene_Combat::End_Battle(E_SCENETAG eTag)
{
	...
	CGameManager::GetInstance()->CheckReaderIndex();
}
...
void CClientScene_Combat::Spawn_Striker()
{
	...
	for (_int i = 0; i < 4; ++i)
	{
		...
		if (pChar)
		{
			if (m_pWorld->Get_WorldMod() != E_WORLD_MOD::MOD_MISSION)
			{
				pChar->Reset_HP();
			}

			if (pChar->Get_UnitDead())
			{
				++iPosCount;
				continue;
			}
...
void CClientScene_Combat::TimeEvent(_float fTimeDelta)
{
	...
	case 0:
	{
		/*_vec2 vUiPos = _vec2(_float(WINCX>>1), _float(WINCY>>1));
		CUISkillObject* pUiSkillObject = CSkill_Manager::GetInstance()->SetUISkill(L"Gacha_Polight.skill", E_LAYERTAG::Effect, vUiPos, 0.f, 0.05f);*/
		Spawn_Special();
