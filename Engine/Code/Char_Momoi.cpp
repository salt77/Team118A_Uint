...
void CChar_Momoi::Set_GuideSkill()
{
	...
	pGuide = CSkill_Manager::GetInstance()->SetSkillGuideFan(Get_Position(), Get_Angle(), 10.f, 45.f, 9999.f, 10, FALSE, L"../../Reference/Resource/Texture/Test/GuideTexBlue.png");
	if (pGuide)
	{
...
void CChar_Momoi::ColMonster(CGameObject * pThis, CGameObject * pMonster)
{
	...
	if (pHitParticle)
	{
		...
		pHitParticle->Set_Scale(_vec3(1.f, 1.f, 1.f));
		pHitParticle->Set_Pos(pMonster->Get_Position());
	}

	static_cast<CUnit*>(pMonster)->Set_Damage(static_cast<CUnit*>(pThis)->Get_UnitInfo(), static_cast<CCharacter*>(pThis)->Get_CharInfo().eAtkType, 3.f);
}
...
void CChar_Momoi::Trigger_Attack_1()
{
	...
	CParticleObject* pShoot = CParticleObject::Create(CGraphicDev::GetInstance()->GetDevice(), E_OBJTAG::OBJ_Particle, L"Momoi_Shoot.particle");
	if (pShoot)
	{
		CScene::Get_MainScene()->Add_GameObject(E_LAYERTAG::Effect, pShoot->Get_ObjTag(), pShoot);
		pShoot->Set_Pos(m_pWeaponCollider->GetPos() + Get_Info(E_INFO::INFO_LOOK) * 0.1f);
		pShoot->Set_Rotation(Get_Angle());
		
	}

	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_JSD_Laser_02.wav", 1.f);
}
...
void CChar_Momoi::Trigger_Attack_3()
{
	...
	CCollisionManager::GetInstance()->Collision_FanToCircle(this, Get_Position(), vLook, 45.f, 10.f,E_LAYERTAG::Monster, ColMonster);
}

void CChar_Momoi::Trigger_ExSkill()
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Skill_Momoi_Ex.wav", 1.f);
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"	Momoi_ExSkill_Level_2.ogg", 1.f);
	if (m_pExsObject)
	{
		m_pExsObject->Set_Dead(TRUE);
		Safe_Release(m_pExsObject);
	}
	m_pExsObject = CSkill_Manager::GetInstance()->SetSkill(L"Momoi_Exs_Shot_0120.skill", E_LAYERTAG::Effect, Get_WeaponPos(0.f, 0.f, 0.f), Get_Angle(), VEC_ONE * 0.5f);
	
	CSkill_Manager::GetInstance()->SetSkill(L"EH_Public_Exs_Effect_0110.skill", E_LAYERTAG::Effect, m_pSphereCollider->GetPos(), Get_Angle(), VEC_ONE);
...
void CChar_Momoi::Trigger_9()
{
	...
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"Momoi_ExSkill_2.ogg", 1.f);
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Skill_Momoi_Ex_Cut_in.wav", 1.f);
