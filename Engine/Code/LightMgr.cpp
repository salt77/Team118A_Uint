...
void CLightMgr::Update_Light_Mgr(const _float & fTimeDelta)
{
	...
	if (m_bLightMove)
	{
		...
		_float fMin = 10.f; // 그림자가 가장 가까울때 
		_float fMax = 60.f; // 그림자가 가장 멀때 
		
		_float fMul = 1.f;
		_float fDis = m_fLightCameraPos_Dis;
		...
		_float fVar = 1.f +	fMul * ((fDis - fMin) / (40.f - fMin));

		// 원본 값 위치 수정 (거리 적용)
		_vec3 vPos = m_pLightCameraPos_Ori * fVar;
		
		LighthtPos = vPos + CCamera::m_pMainCam->Get_CamAt();
	}
