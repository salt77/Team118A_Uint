...
_int CHieronymus::Start_Object(const _float & fTimeDelta)
{
	...
	CScene::Get_MainScene()->Add_GameObjectNextFrame_ForLoading(E_LAYERTAG::Effect, E_OBJTAG::OBJ_Particle, pParticle);
...
_int CHieronymus::Update_Object(const _float & fTimeDelta)
{
	...
	m_fExSkill2_Gage -= fTimeDelta * 0.06f;
...
void CHieronymus::Ex1_Pattern(_int iRand, _bool bStart)
{
	if (iRand < 50)
	{
		...
		{
			Hieronymus_Impact_1();
			CScene::Get_MainScene()->Active_ShakeCam(0.5f, 0.2f);
		}
	}
	else if (iRand < 100)
	{
...
void CHieronymus::Ex3_Pattern(_int iRand, _bool bStart)
{
		if (iRand < 50)
		{
			...
			else
			{
				if (m_pTarget)
				{
					m_vImpact2Pos = m_pTarget->Get_Position();
...
void CHieronymus::Test_Bullet_6()
{
	...
	for (_int i = 0; i < (_int)vStrikersPos.size(); ++i)
	{
		...
		{
			CScene::Get_MainScene()->Add_GameObjectNextFrame_ForLoading(E_LAYERTAG::AttackObj_Monster, E_OBJTAG::OBJ_AtkObj, iter);
		}
...
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Hieronymus_EX4_Explosion.wav", 1.f);
	Invoke_Add(2.f, bind(&CHieronymus::Hieronymus_Impact_2_Shake, this));
}

void CHieronymus::Hieronymus_Impact_2_Shake()
{
	CScene::Get_MainScene()->Active_ShakeCam(1.0f, 0.1f);
	...
	{
		if (CUtility::Distance_vec3(iter->Get_Position(), m_vImpact2Pos) <= 3.f)
		{
...
void CHieronymus::Rand_Warn(_bool bRand)
{
	...
	case E_RandWarn::Rand_Bomb:				// 그냥 무작위 보라색
	{
		for (_int i = 0; i < 25; ++i)
		{
			...
			CScene::Get_MainScene()->Add_GameObjectNextFrame_ForLoading(E_LAYERTAG::Effect, E_OBJTAG::OBJ_Effect, pWarn);
			pWarn = nullptr;
		}
	}
		break;
	...
	case E_RandWarn::_3:					// 5개 앞으로 진행하면서 오는거?
	{
		pWarn = CWarning::Create(m_pGraphicDev);
		pWarn->Set_Damage(150);
		pWarn->Set_AfterSkillName(L"Hieronymus_Warn_Posion_00.skill", VEC_ZERO, VEC_ONE * 2.f);
		pWarn->Set_SoundName(L"SFX_Electric_JSD_Shock_02.wav", 1.f);
		pWarn->Add_CombatStatus(T_CombatStatus(E_Combat_Status::CC_Stunned, 3.f));
	...
	_int iWarn = (_int)m_eRandWarn;
	if (iWarn == (_int)E_RandWarn::RandWarn_END)
		m_eRandWarn = E_RandWarn::Rand_Bomb;
	else
	{
		iWarn++;
		m_eRandWarn = (E_RandWarn)iWarn;
...
void CHieronymus::Lava_Invoke()
{
	...
	CWarning_Mgr::GetInstance()->Create_Line_TwoWay(pWarn, fRandAngle, 1.5f, 4, 0.0f, 0.1f, vPos, C_White, C_White);
	Safe_Release(pWarn);
}
...
void CHieronymus::ViewCut_Update(const _float& fTimeDelta)
{
	...
	_float fRatio = 1.f - (m_fCutViewCoolTime / m_fCutViewOnce);
	...
	if (m_fCutViewOnce <= m_fCutViewCoolTime)
	{
		T_ViewCut tViewCut = T_ViewCut();

		m_fCutViewOnce = m_fCutViewTotalTime / m_iCutViewTime;
		if (m_iCutViewStack < 3)
		{
			m_fCutViewOnce *= 2.2f;
		}
		else if (m_iCutViewStack < 8)
		{
			m_fCutViewOnce *= 1.6f;
		}
		else if(m_iCutViewStack + 1 == m_iCutViewTime)
		{
			m_fCutViewOnce *= 6.f;
		}
		else
		...
		tViewCut.fTimeMax	= m_fCutViewOnce;
		tViewCut.fTime		= tViewCut.fTimeMax * 0.5f;

		if (m_iCutViewStack % 3 == 0)
		{
			tViewCut.fDis = CWall512::GetInstance()->GetFloatValue(-4.f, -0.5f);
		}
		...
		{
			tViewCut.fDis = CWall512::GetInstance()->GetFloatValue(0.5, 4.f);
		}
		...
		else
		{
			...
			_float fA = m_fCutViewTilt;
			_float fC = sqrt((m_fCutViewTilt * m_fCutViewTilt) + 1.f);
			fAngle = acosf(fA / fC) * Rad2Deg;
			fAngle = 180.f - fAngle;
			...
			static_cast<CEffectObject*>(pUiSkillObject->Get_vecSkillObjects()->front())->Get_Effect_Info().fLifeTime = m_fCutViewOnce * (1.f + m_iCutViewTime - m_iCutViewStack);
			...
			CRenderer::GetInstance()->Set_ViewCut_Fix(m_fCutViewOnce, tViewCut);

			_int iSoundRand = CWall512::GetInstance()->GetValue(0, 3);
			if (m_iCutViewStack == m_iCutViewTime)
			{
				CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Skill_Neru_Ex_Cut_in.wav", 1.5f);
			}
			if (iSoundRand == 0)
			{
				CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_BiFrost_01.wav", 1.5f);
			}
			else if(iSoundRand == 1)
			{
				CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_BiFrost_02.wav", 1.5f);
			}
			else
			{
				CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_BiFrost_03.wav", 1.5f);
			}
		}
	}
...
void CHieronymus::Set_SkySphere(wstring wstrSkyTexFullPath)
{
	if (!m_pSkySphere)
	{
		...
		CScene::Get_MainScene()->Add_GameObjectNextFrame_ForLoading(E_LAYERTAG::GameLogic, E_OBJTAG::OBJ_BackGround, m_pSkySphere);
		m_pSkySphere->Set_Scale(20.f, 20.f, 20.f);
	}
	else
	{
