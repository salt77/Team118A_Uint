...
void CUI_Manager::Create_Ui_CharCollection()
{
	...
	if (m_pCollectionPanel)
	{
		m_pCollectionPanel->Set_Dead(TRUE);
		m_pCollectionPanel = nullptr;
	}
...
void CUI_Manager::Open_Ui_BattleFormation(_int eWorldMod)
{
	...
	for (auto& tPair : m_mapCharBtn)
	{
		if (CCharacterMgr::GetInstance()->Get_CharInfo(tPair.first).bGetChar)
		{

			tPair.second->Set_Active(TRUE);
		}
		else
		{
			tPair.second->Set_Active(FALSE);
		}

		wstring wstrLevelText = L"Lv." + to_wstring(CCharacterMgr::GetInstance()->Get_CharInfo(tPair.first).iLevel);
		for (_int i = 0; i < 5; ++i)
		{
			tPair.second->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(i)->Set_Ui_Text(wstrLevelText);
		}
...
void CUI_Manager::BattleFormation_SetWindow(CUI_Object * pUIObj, const T_CharInfo & tCharInfo)
{
	...
	if (pUIObj->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildList()->empty())
	{
	...
	for (_int i = 0; i < 5; ++i)
	{
		pUIObj->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(0)->Get_ChildByIndex(i)->Set_Ui_Text(wstrLevelText);
	}
...
void CUI_Manager::Create_Ui_MissionPanel()
{
	...
	for (auto& iter : (*pWorlds))
	{
		...
		for (auto& iterWorld : iter.second)
		{
			...
			wstring wstrStageName = iterWorld->Get_WorldName().substr(0, iterWorld->Get_WorldName().find(L"."));
			pStage->Get_ChildByIndex(4)->Get_ChildByIndex(0)->Set_Ui_Text(wstrStageName);
	...
	for (_int i = 0; i < (_int)vecTileFiles.size(); ++i)
	{
		CUI_Object* pStage = Load_UI_Prefab(L"../../Reference/Data/Data_UI/Prefab/[Fixed]스테이지 Prefab.uidat");
		pStage->Get_ChildByIndex(0)->Set_Ui_Text(to_wstring(iCount));
		wstring wstrStageName = vecTileFiles[i].substr(0, vecTileFiles[i].find(L"."));
		pStage->Get_ChildByIndex(4)->Get_ChildByIndex(0)->Set_Ui_Text(wstrStageName);
...
void CUI_Manager::Mission_Start()
{
	...
	_int iRandBGM = CWall512::GetInstance()->GetValue(0, 3);
	if (iRandBGM == 0)
	{
		CSound_Manager::GetInstance()->PlayBGM(L"BGM_전투_임무1.mp3", 0.1f);
	}
	else if (iRandBGM == 1)
	{
		CSound_Manager::GetInstance()->PlayBGM(L"BGM_전투_임무2.mp3", 0.1f);
	}
	else if (iRandBGM == 2)
	{
		CSound_Manager::GetInstance()->PlayBGM(L"BGM_전투_임무3.mp3", 0.1f);
	}
...
void CUI_Manager::Mission_ClickEnemyInfo()
{
	...
	for (_int i = 0; i < iScrollChildSize; ++i)
	{
		...
		if (CXL_ParsingMgr::GetInstance()->Translation_EnumToString((_int)tInfo.eAtkType, E_CHAR_ENUMTYPE::CNUM_ATKTYPE) == L"NULL")
		{
			pInfo->Get_ChildByIndex(1)->Set_Ui_Text(L"노말");
		}
		else
		{
		...
		if (CXL_ParsingMgr::GetInstance()->Translation_EnumToString((_int)tInfo.eDiffType, E_CHAR_ENUMTYPE::CNUM_DFFTYPE) == L"NULL")
		{
			...
			pInfo->Get_ChildByIndex(1)->Set_Ui_Text(L"기본");
		}
		else
		{
			pInfo->Get_ChildByIndex(2)->Set_Ui_Text(CXL_ParsingMgr::GetInstance()->Translation_EnumToString((_int)tInfo.eDiffType, E_CHAR_ENUMTYPE::CNUM_DFFTYPE));
...
void CUI_Manager::OnClick_TotalInfoPanel(_int iBossType)
{
	...
	case CUI_Manager::E_TotalPanel_Binah:
		...
		m_pTotalInfo_Frame_BossSkill_5->Set_Ui_Text(L"정화의 폭풍");
	...
	case CUI_Manager::E_TotalPanel_Hieronymus:
		...
		for (_int i = 0; i < 5; ++i)
		{
			m_pTotalInfo_HealthText_OutLine->Get_ChildByIndex(0)->Get_ChildByIndex(i)->Set_Ui_Text(L"400000 / 400000");
		}

		// 스킬 5개 세팅
		m_pTotalInfo_Frame_BossSkill_1->Set_Ui_Text(L"다마소의 부름");
		...
		m_pTotalInfo_Frame_BossSkill_2->Set_Ui_Text(L"불가타의 경전");
		...
		m_pTotalInfo_Frame_BossSkill_3->Set_Ui_Text(L"사자의 구제");
		...
		m_pTotalInfo_Frame_BossSkill_4->Set_Ui_Text(L"사막의 고행");
		...
		m_pTotalInfo_Frame_BossSkill_5->Set_Ui_Text(L"성스러운 빛");
		m_pTotalInfo_Frame_BossSkill_5->Get_ChildByIndex(1)->Set_Tex1_Tag(L"../../Reference/Resource/Texture/UI/Skill/Binah_ExSkill_1.png");
	...
	case CUI_Manager::E_TotalPanel_Chesed:
		...
		for (_int i = 0; i < 5; ++i)
		{
			m_pTotalInfo_HealthText_OutLine->Get_ChildByIndex(0)->Get_ChildByIndex(i)->Set_Ui_Text(L"800000 / 800000");
		}

		// 스킬 5개 세팅
		m_pTotalInfo_Frame_BossSkill_1->Set_Ui_Text(L"몬스터 소환");
		m_pTotalInfo_Frame_BossSkill_1->Get_ChildByIndex(1)->Set_Tex1_Tag(L"../../Reference/Resource/Texture/UI/Skill/Chesed_ExSkill.png");

		m_pTotalInfo_Frame_BossSkill_2->Set_Ui_Text(L"탄환 레이저");
		...
		m_pTotalInfo_Frame_BossSkill_3->Set_Ui_Text(L"추적 레이저");
		...
		m_pTotalInfo_Frame_BossSkill_4->Set_Ui_Text(L"탄막의 비");
		...
		m_pTotalInfo_Frame_BossSkill_5->Set_Ui_Text(L"심판자");
		m_pTotalInfo_Frame_BossSkill_5->Get_ChildByIndex(1)->Set_Tex1_Tag(L"../../Reference/Resource/Texture/UI/Skill/Hieronymus_ExSkill_1.png");
	...
	case CUI_Manager::E_TotalPanel_Kuro:
		...
		for (_int i = 0; i < 5; ++i)
		{
			m_pTotalInfo_HealthText_OutLine->Get_ChildByIndex(0)->Get_ChildByIndex(i)->Set_Ui_Text(L"350000 / 350000");
		}

		// 스킬 5개 세팅
		m_pTotalInfo_Frame_BossSkill_1->Set_Ui_Text(L"'콰쾅'");
		...
		m_pTotalInfo_Frame_BossSkill_2->Set_Ui_Text(L"'굴려라 굴려!'");
		...
		m_pTotalInfo_Frame_BossSkill_3->Set_Ui_Text(L"'돌진이다!'");
		...
		m_pTotalInfo_Frame_BossSkill_4->Set_Ui_Text(L"'까마귀 소환'");
		...
		m_pTotalInfo_Frame_BossSkill_5->Set_Ui_Text(L"'지형파괴'");
		m_pTotalInfo_Frame_BossSkill_5->Get_ChildByIndex(1)->Set_Tex1_Tag(L"../../Reference/Resource/Texture/UI/Skill/Binah_ExSkill_1.png");
		break;
	}

	m_pTotalInfo_Recommand_Level_OutLine->Set_Ui_Text(L"Lv." + to_wstring(iBossLevel));
...
void CUI_Manager::Active_Skill(_bool bActive)
{
	...
	if (bActive)
	{
		if (m_pCombatIng_FocusUnit)
		{
			static_cast<CCharacter*>(m_pCombatIng_FocusUnit)->Cancel_Aiming();
		}
		return;
	}
...
void CUI_Manager::Setting_Combat_Victory()
{
	...
	m_pCombatVictoryPanel->Get_ChildByIndex(2)->Get_ChildByIndex(0)->Get_ChildByIndex(1)->Get_ChildByIndex(0)->Set_Ui_Text(L"0/" + to_wstring(CGameManager::GetInstance()->Get_Level() * 10));
...
void CUI_Manager::Create_GachaResult_UI()
{
	...
	// 가챠로 뽑을 수 있는 유닛 추가
	m_vecGachaUnit_R.push_back((_int)E_CHAR_NAME::Name_Chinatsu);
	m_vecGachaUnit_R.push_back((_int)E_CHAR_NAME::Name_Fuuka);
	m_vecGachaUnit_R.push_back((_int)E_CHAR_NAME::Name_Juri);
	m_vecGachaUnit_R.push_back((_int)E_CHAR_NAME::Name_Nodoka);
	m_vecGachaUnit_R.push_back((_int)E_CHAR_NAME::Name_Yoshimi);
	...
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Kayoko);
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Airi);
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Utaha);
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Chise);
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Hasumi);
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Serika);
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Akane);
	m_vecGachaUnit_SR.push_back((_int)E_CHAR_NAME::Name_Mutsuki);

	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Neru);
	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Azusa);
	...
	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Hibiki);
	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Karin);
	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Mashiro);
	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Izumi);
	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Maki);
	m_vecGachaUnit_SSR.push_back((_int)E_CHAR_NAME::Name_Yuzu);
}
...
void CUI_Manager::Change_GachaResultType(E_GachaResultType ResultType)
{
	...
	case E_GachaResultType::FinalResult:
		m_pGachaResult_BlackMask->Set_Active(FALSE);
		m_pGachaResult_BlackMask->Set_ImageAlpha(0.f);
		...
		for (_int i = 0; i < 10; i++)
		{
			...
			else
			{
				...
				CCharacterMgr::GetInstance()->Get_vecCharInfo()[m_iArrGachaResultUnit[i]].bGetChar = TRUE;
			}
