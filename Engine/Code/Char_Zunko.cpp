...
_int CChar_Zunko::LateUpdate_Object(const _float & fTimeDelta)
{
	...
	if (m_pGroundObject)
	{
		...
		CCollisionManager::GetInstance()->Collision_StraightToCircle(this, Get_Position(), Get_Info(E_INFO::INFO_LOOK), 1.5f, m_fExDistance, E_LAYERTAG::Monster, ColMonster);
	}
...
void CChar_Zunko::ColEx(CGameObject * pMonster)
{
	...
	static_cast<CUnit*>(pMonster)->Set_Damage(m_tUnitInfo, m_tCharInfo.eAtkType, 12.f);
}

void CChar_Zunko::Set_GuideSkill()
{
	CSkillGuideObject* pGuide = nullptr;
	pGuide = CSkill_Manager::GetInstance()->SetSkillGuideStraight(Get_Position(), Get_Angle(), 10.f, 1.5f, 9999.f, FALSE, L"../../Reference/Resource/Texture/Test/GuideTexBlue.png");
	if (pGuide)
	{
...
void CChar_Zunko::Trigger_Attack_1()
{
	...
	CParticleObject* pShoot = CParticleObject::Create(CGraphicDev::GetInstance()->GetDevice(), E_OBJTAG::OBJ_Particle, L"ZunkoShoot.particle");
	if (pShoot)
	{
	...
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Common_Shot_Single_01.wav", 1.f);
}

void CChar_Zunko::Trigger_ExSkill()
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"Zunko_ExSkill_Level_3.ogg", 1.f);
	
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Skill_Zunko_Ex.wav", 1.f);
	if (m_pShootObject)
	{
	...
	m_pShootObject = CSkill_Manager::GetInstance()->SetSkill(L"CharSkill_Zunko_Exs_Shot.skill", E_LAYERTAG::Effect, Get_WeaponPos(0.f, 0.f, 0.f), Get_Angle(), VEC_ONE);
	if (m_pShootObject)
	{
	...
	m_pGroundObject = CSkill_Manager::GetInstance()->SetSkill(L"CharSkill_Zunko_Exs_Ground.skill", E_LAYERTAG::Effect, Get_Position(), Get_Angle(), VEC_ONE);
	if (m_pGroundObject)
	{
...
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Skill_Zunko_Ex_Cut_in.wav", 1.f);
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"Zunko_ExSkill_1.ogg", 1.f);
