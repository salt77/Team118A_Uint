...
_int CKuro::Start_Object(const _float & fTimeDelta)
{
	...
	m_pBossHpBar = CBoss_HpBar::Create(m_pGraphicDev, CUI_Manager::GetInstance()->Get_Hwnd(), L"Kuro", m_tUnitInfo.iMaxHp, 20);
	assert(m_pBossHpBar);
...
void CKuro::Invoke_FireShoot()
{
	...
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SE_FireWorks_02.wav", 1.f);
...
void CKuro::FireShoot()
{
	...
	vRot.x = 0.f;
	m_tFireShootGuide = CSkill_Manager::GetInstance()->SetSkillGuideFan(Get_Position(), vRot, 8.f, 240.f, 3.f, 50, TRUE)->Get_GuideOption();
...
{
	CMonster* pMonster = static_cast<CMonster*>(pThis);
	static_cast<CUnit*>(pPlayer)->Set_Damage(pMonster->Get_UnitInfo(), pMonster->Get_MonsterInfo().eAtkType, 0.45f);
}
