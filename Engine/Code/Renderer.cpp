...
{	
	...
	//pEffect->SetBool("g_bRenderDebug", false);
	...
	if (m_bViewCut) 
	{
		...
		// ViewCut 셋팅
		pEffect = m_pPostProcessShader->Get_EffectHandle();
		pEffect->SetFloat("g_fCutLine_Dir", -1.f);
		//pEffect->SetBool("g_bRenderDebug", true);
		//
		//pEffect = m_pBlendShader->Get_EffectHandle();
		//pEffect->SetFloat("g_fCutLine_Dir", -1.f);
		//pEffect->SetBool("g_bRenderDebug", true);
...
void CRenderer::RenderOption(const _float & fTimeDelta)
{
	...
	if (CKeyManager::GetInstance()->Key_Pressing(KEY_Shift) && CKeyManager::GetInstance()->Key_Pressing(KEY_Alt))
	{
		...
		if (CKeyManager::GetInstance()->Key_Down(KEY_B))
		{
			...
			m_tViewCutFix.fTimeMax = 1.f;
			m_tViewCutFix.fTime = 1.f;
			m_tViewCutFix.fDis = 0.f;
			...
			if (m_bRenderDebug) 
			{
				...
				m_fTime_ViewCutFix = m_fRenderDebug_Time;
			}
			else
			{
				m_bRenderDebug = TRUE;
				m_fTime_ViewCutFix = 60.f;
			}
		}
	}
...
void Engine::CRenderer::Render_UI(LPDIRECT3DDEVICE9& pGraphicDev)
{
	...
	{
		CFontMgr::GetInstance()->Render_FontEveryFrame(T_RENDERFONT(FontTag_경기천년제목_Bold, L"PostProcess Off", _vec2(-WINCX * 0.5f, -WINCY * 0.5f), D3DXCOLOR(1.f, 1.f, 1.f, 1.f), 15, 15, FW_NORMAL, FALSE));
		CFontMgr::GetInstance()->Render_FontEveryFrame(T_RENDERFONT(FontTag_경기천년제목_Bold, L"PostProcess On", _vec2(0.f, -WINCY * 0.5f), D3DXCOLOR(1.f, 1.f, 1.f, 1.f), 15, 15, FW_NORMAL, FALSE));
	}
}
...
void CRenderer::Render_Debug()
{
	...
	CRenderTargetMgr::GetInstance()->Render_DebugBuffer(L"MRT_Effect_Blend");
...
void CRenderer::Reverse_Debug()
{
	...
	//m_bRenderDebug_Table = !m_bRenderDebug_Table;
...
void CRenderer::Update_ViewCut(const _float & fTimeDelta)
{
	...
	pEffect = m_pBaseShader->Get_EffectHandle();
	pEffect->SetFloat("g_fCutLine_Begin", fLineBegin);
	pEffect->SetFloat("g_fCutLine_End", fLineEnd);
}

void CRenderer::Update_RenderDebug(const _float & fTimeDelta)
{
	// 랜더 디버그 컷뷰 업데이트
	...
	if (m_fRenderDebug_Time < m_fRenderDebug)
	{
		m_fRenderDebug = m_fRenderDebug_Time;
	}
	if (0.f > m_fRenderDebug)
	{
		m_fRenderDebug = 0.f;
	...
	if (m_fRenderDebug_Time > m_fTime_ViewCutFix)
	{
		m_bRenderDebug = false;
	}
	...
	if (0.f < m_fRenderDebug)
	{
		_float fRatio = m_fRenderDebug / m_fRenderDebug_Time;
		...
		m_tViewCutFix.fTime = (1.f - fRatio) * 0.5f + 0.5f;
	}
}

void CRenderer::Set_ViewCut_Fix(_float fTime, T_ViewCut tViewcutFix)
{
	if (!m_bRenderDebug && 0.f == m_fRenderDebug) 
	{
		m_fTime_ViewCutFix = fTime;
		m_tViewCutFix = tViewcutFix;
	}
}

T_ViewCut& CRenderer::Get_ViewCut_Fix()
{
	return m_tViewCutFix;
}
