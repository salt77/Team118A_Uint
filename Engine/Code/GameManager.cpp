...
void CGameManager::CheckReaderIndex()
{
	//m_eCurrentSquad
	if (Get_CurrentSquad_Alive_Strikers().empty())
	{
		return;
	}

	_int iIndex = 0;
	for (auto& pChar : (*m_mapCurrentSquad)[(_int)E_CHAR_COMBATTYPE::COMBATTYPE_STRIKER])
	{
		if (!pChar)
		{
			++iIndex;
		...
		{
			m_iReaderIndex[(_int)m_eCurrentSquad] = iIndex;
			break;
		}		
	}
}
...
void CGameManager::Initialize_SkillOrder()
{
	...
	while (FALSE == m_qSkillOrder.empty())
	{
		m_qSkillOrder.pop();
	}
	...
	if (iMaxSize == 4)
	{
		m_qSkillOrder.push(m_vecBattleSkillOrder[3]);
