...
void CChar_Shun::Trigger_1()
{
	auto pVec = CGameManager::GetInstance()->Get_CurrentSquad_Alive_Strikers();
	for (_int i = 0; i < (_int)pVec.size(); ++i)
	{
		if (static_cast<CUnit*>(pVec[i])->Get_UnitDead())
			continue;

		static_cast<CUnit*>(pVec[i])->Add_CombatStatus(T_CombatStatus(E_Combat_Status::B_DEF, 5.f));
		static_cast<CUnit*>(pVec[i])->Add_CombatStatus(T_CombatStatus(E_Combat_Status::B_OppressionPower, 5.f));
	}
}
