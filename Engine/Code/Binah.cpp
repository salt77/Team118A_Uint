...
void CBinah::Free()
{
	...
	{
		m_pEye_L1->Set_Dead(true);
		Safe_Release(m_pEye_L1);
		m_pEye_L1 = nullptr;
	}

	if (m_pEye_L2 != nullptr)
	{
		m_pEye_L2->Set_Dead(true);
		Safe_Release(m_pEye_L2);
		m_pEye_L2 = nullptr;
	}
}
...
HRESULT CBinah::Ready_Object()
{
	...
	wstring	m_wstrUnitName2 = L"Hod_Tower";
	wstring wstrFullPath2 = L"../../Reference/Resource/Monster/" + m_wstrUnitName2 + L"/";
	wstring wstrFileName2 = m_wstrUnitName2 + L".X";
	if (E_FAIL == CProtoMgr::GetInstance()->Ready_Prototype(m_wstrUnitName2,
		CDMesh::Create(m_pGraphicDev, wstrFullPath2.c_str(), wstrFileName2.c_str())))
	{
		assert(FALSE);
	}
	wstring	m_wstrUnitName3 = L"Hod_Cannon";
	wstring wstrFullPath3 = L"../../Reference/Resource/Monster/" + m_wstrUnitName3 + L"/";
	wstring wstrFileName3 = m_wstrUnitName3 + L".X";
	if (E_FAIL == CProtoMgr::GetInstance()->Ready_Prototype(m_wstrUnitName3,
		CDMesh::Create(m_pGraphicDev, wstrFullPath3.c_str(), wstrFileName3.c_str())))
	{
		assert(FALSE);
	}
	...
	m_pBossHpBar = CBoss_HpBar::Create(m_pGraphicDev, CUI_Manager::GetInstance()->Get_Hwnd(), L"Binah", m_tUnitInfo.iMaxHp, 20);
	assert(m_pBossHpBar);
...
_int CBinah::Update_Object(const _float & fTimeDelta)
{
	...
	if (m_fIntroNameTime >= 14.3f)
	{
		if (m_bIntroNameSound)
		{
			CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Binah_Intro_Name.wav", 3.f);
			m_bIntroNameSound = false;
		}
	}

	if (m_fActiveCharTime >= 14.8f)
	{
		if (m_bAutoCam)
		{
...
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Binah_Intro1.wav", 1.f);
...
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Binah_Intro2.wav", 1.f);
...
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Binah_Intro3.wav", 1.f);
...
{
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Binah_Intro_Eye.wav", 1.f);
}
