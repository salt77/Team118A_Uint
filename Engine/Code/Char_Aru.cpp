...
void CChar_Aru::Create_HitEffect()
{
	if (E_UNIT_HIT_TYPE::HIT_EXS == m_eUnitHitType) 
	{
		...
		else 
		{
			if (m_pTarget) 
			{
				m_vPos_Exs = m_pTarget->Get_Position();
			}
			else
			{
				m_vPos_Exs = Get_Position();
...
void CChar_Aru::AruBomb()
{
	...
	// 폭팔 광음
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Hieronymus_NormalAttack_Hit_Start.wav", 6.f);


	m_tUnitInfo.iAttack = 300;
	//_vec3 vLook = Get_Info(E_INFO::INFO_LOOK);
	//D3DXVec3Normalize(&vLook, &vLook);
	//CCollisionManager::GetInstance()->Collision_FanToCircle(this, Get_Position(), vLook, 25.f, 9.f, E_LAYERTAG::Monster, ColMonster);
	CCollisionManager::GetInstance()->Collision_SpotToCircle(this, m_vPos_Exs, 3.f, E_LAYERTAG::Monster, ColMonster);

	m_vPos_Exs = Get_Position();
	m_vRot_Exs = _vec3();
}

void CChar_Aru::ColMonster(CGameObject * pThis, CGameObject * pMonster)
{
	// 몬스터에게 데미지를 준다
