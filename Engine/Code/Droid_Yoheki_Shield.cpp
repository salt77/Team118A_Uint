...
_int CDroid_Yoheki_Shield::Update_Object(const _float & fTimeDelta)
{
	...
	if (m_pEffectShield)
	{
		...
		m_pEffectShield->Set_Effect_Pos(m_pShieldCollider->GetPos() - m_pShieldCollider->GetLook() * 0.2f); 
	}
...
void CDroid_Yoheki_Shield::Trigger_1()
{
	...
	static_cast<CUnit*>(this)->Add_CombatStatus(T_CombatStatus(E_Combat_Status::DB_DEF, 6.f)); 
