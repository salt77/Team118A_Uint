...
void CActionCam_Mgr::MoveOn_Line()
{
	...
	{
		//if ((_uint)m_dbMapActionEye[m_wstrCurFile][m_wstrCurEvent].size() - 2 > m_iCurLineIndex)
		//{
		//	m_fLineLength = D3DXVec3Length(&(m_dbMapActionEye[m_wstrCurFile][m_wstrCurEvent][m_iCurLineIndex]->Get_Info(E_INFO::INFO_POS) -
		//									 m_dbMapActionEye[m_wstrCurFile][m_wstrCurEvent][m_iCurLineIndex + 1]->Get_Info(E_INFO::INFO_POS)));
		...
		//if (m_dbMapSectionSpeed[m_wstrCurFile][m_wstrCurEvent].empty())
		//{
		//	m_fProgressCam += 100.f * m_fTimeDelta;
		//}
		//else
		//{
		//	m_fProgressCam += 10.f * m_dbMapSectionSpeed[m_wstrCurFile][m_wstrCurEvent][m_iCurLineIndex] * m_fTimeDelta;
		//}
	}
}
...
{
	vector<CCamControl_Eye*>	vecTemp = m_dbMapActionEye[m_wstrCurFile][m_wstrCurEvent];
	...
	if (3 < vecTemp.size())
	{
			...
			m_iCurLineIndex < vecTemp.size() - 2)
		{
			Make_Spline(&m_vLinePos,
						Get_Adjust_CamPos(vecTemp[m_iCurLineIndex - 1]->Get_Info(E_INFO::INFO_POS)),
						Get_Adjust_CamPos(vecTemp[m_iCurLineIndex]->Get_Info(E_INFO::INFO_POS)),
						Get_Adjust_CamPos(vecTemp[m_iCurLineIndex + 1]->Get_Info(E_INFO::INFO_POS)),
						Get_Adjust_CamPos(vecTemp[m_iCurLineIndex + 2]->Get_Info(E_INFO::INFO_POS)));
		}
...
{
	vector<CCamControl_At*>		vecTemp = m_dbMapActionAt[m_wstrCurFile][m_wstrCurEvent];
	...
	if (1 == vecTemp.size())
	{
		if (E_ActionCam_Mode::ActionCam_Subject == m_eCurActionMode)
		{
			m_pMainCam->Set_CamAt(Get_Adjust_CamPos(vecTemp[0]->Get_Info(E_INFO::INFO_POS)));
		}
	}
}
