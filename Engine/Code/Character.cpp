...
void CCharacter::Begin_Battle(_float fDeltaTime)
{
	if (m_eObjTag != E_OBJTAG::OBJ_Special)
	{
		...
		{
			if (m_eTargetLayerTag == E_LAYERTAG::Character)
			{
				Set_Play(TRUE);
				return;
			}
...
_int CCharacter::Set_Damage(const T_UnitInfo & tEnemyInfo, E_CHAR_ATKTYPE eEnemyAtkType, _float fRatio)
{
	_int iDamage = CUnit::Set_Damage(tEnemyInfo, eEnemyAtkType, fRatio);
