...
CUnit* CWorldCreator::NameTranslateSpawn(wstring wstrName, _vec3 vPos, _vec3 vRot, T_MONSTER_INFO& tMonsterInfo)
{
	...
	else if (wstrName == L"Hod_Tower")								pUnit = CHod_Tower::Create(CGraphicDev::GetInstance()->GetDevice(), E_OBJTAG::OBJ_Monster);
	else if (wstrName == L"Hod_Cannon")								pUnit = CHod_Cannon::Create(CGraphicDev::GetInstance()->GetDevice(), E_OBJTAG::OBJ_Monster);
